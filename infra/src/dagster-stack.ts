/**
 * Optional Dagster EC2 stack (always-on cost). Deploy only when needed.
 *
 *   cdk deploy --all --profile … -c deployDagster=true
 */
...
import { DAGSTER_EC2_INSTANCE_SIZE } from "./config.js";
import { PolitikerkollenCoreStack } from "./core-stack.js";

const RESOURCE_PREFIX = "politikerkollen";

export interface PolitikerkollenDagsterStackProps extends cdk.StackProps {
  core: PolitikerkollenCoreStack;
}

export class PolitikerkollenDagsterStack extends cdk.Stack {
  public readonly dagsterRepo: ecr.Repository;
  public readonly instance: ec2.Instance;

  constructor(scope: Construct, id: string, props: PolitikerkollenDagsterStackProps) {
    super(scope, id, props);

    const { core } = props;

    this.dagsterRepo = new ecr.Repository(this, "DagsterRepo", {
      repositoryName: `${RESOURCE_PREFIX}/dagster`,
    ...
    const dagsterSg = new ec2.SecurityGroup(this, "DagsterSg", {
      vpc: core.vpc,
      securityGroupName: `${RESOURCE_PREFIX}-dagster-sg`,
    ...
    const dagsterPostgresSecret = new secretsmanager.Secret(this, "DagsterPostgresSecret", {
      secretName: `${RESOURCE_PREFIX}/dagster-postgres`,
    ...
    const ec2Role = new iam.Role(this, "Ec2Role", {
      roleName: `${RESOURCE_PREFIX}-dagster-ec2-role`,
    ...
    core.motherDuckSecret.grantRead(ec2Role);
    dagsterPostgresSecret.grantRead(ec2Role);
    this.dagsterRepo.grantPullPush(ec2Role);
    core.ingestionRepo.grantPullPush(ec2Role);
    core.dbtRepo.grantPullPush(ec2Role);
    core.cognitionRepo.grantPullPush(ec2Role);
    ...
    userData.addCommands(
      ...
      "",
      "# Get secrets",
      `MOTHERDUCK_TOKEN=$(aws secretsmanager get-secret-value --secret-id ${core.motherDuckSecret.secretName} --query SecretString --output text | jq -r .token)`,
...
SERVICE`,
    ...
    this.instance = new ec2.Instance(this, "DagsterInstance", {
      vpc: core.vpc,
      vpcSubnets: { subnetType: ec2.SubnetType.PUBLIC, onePerAz: false },
      instanceType: ec2.InstanceType.of(ec2.InstanceClass.T4G, DAGSTER_EC2_INSTANCE_SIZE),
    ...
    cdk.Tags.of(this.instance).add("Name", `${RESOURCE_PREFIX}-dagster`);

    new cdk.CfnOutput(this, "InstanceId", { value: this.instance.instanceId });
    new cdk.CfnOutput(this, "DagsterUrl", {
