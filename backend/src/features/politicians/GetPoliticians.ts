...
import { listPoliticians, getBatchMotionStats, getBatchTopRebelTopics, getBatchAccountabilityStats, getConstituencies } from './repository';
...
export async function handleListPoliticians(params: ListPoliticiansParams): Promise<PaginatedPoliticiansResponse> {
  ...
  const [motionStatsMap, topRebelTopicsMap, accountabilityStatsMap] = await Promise.all([
