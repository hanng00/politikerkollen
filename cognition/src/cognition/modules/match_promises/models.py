...
ALIGNMENT_VALUES = Literal["supports", "opposes", "tangential", "irrelevant"]
...
ALIGNMENT_DESCRIPTIONS = {
    ...
    "irrelevant": "The document has no meaningful connection to the promise — the retrieval was a false positive",
}
...
class PromiseVoteMatch(BaseModel):
    ...
    alignment: str | None = Field(
        default=None,
        description="Alignment classification: supports, opposes, tangential, or irrelevant",
    )
    alignment_confidence: float | None = Field(
        default=None, description="Confidence in alignment classification (0-1)"
    )
...
Given a political promise and a parliamentary document (motion or proposition), classify the relationship between them.
...
- Use "irrelevant" when the document and promise share no substantive policy connection — e.g. they happen to mention the same broad area but address completely different issues. This is a retrieval quality gate: false positives should be rejected.
