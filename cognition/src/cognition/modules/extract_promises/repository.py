...
def ensure_tables_exist(conn: duckdb.DuckDBPyConnection) -> None:
    """Create the output tables if they don't exist, and add any missing columns."""
    ...
    # Full column list for schema migration
    promises_columns = [
        ("promise_id", "VARCHAR PRIMARY KEY"),
        ("document_id", "VARCHAR NOT NULL"),
        ("party_id", "VARCHAR"),
    ...
    ensure_columns_exist(conn, PROMISES_TABLE, promises_columns)
    ...
    """)
    ensure_columns_exist(conn, STATE_TABLE, state_columns)
...
def get_document_count(
    ...
    try:
        if year:
            ...
            processed = conn.execute(f"SELECT COUNT(*) FROM {STATE_TABLE}").fetchone()[
                0
            ]
...
def save_promises(
    ...
    for promise in result.promises:
        ...
        model_values = {field: getattr(promise, field) for field in model_field_names}
        rows.append(
            {
    ...
    conn.register("_promise_batch", table)
    conn.execute(
        f"INSERT INTO {PROMISES_TABLE} ({columns_sql}) SELECT {columns_sql} FROM _promise_batch",
    )
    conn.unregister("_promise_batch")

    _mark_document_processed(
        conn, result.document_id, model_version, len(result.promises), cost_usd
    )
